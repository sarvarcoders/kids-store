...
export function formatChannelPost(
  ...
  const sizesText = truncateTelegramText(
    sizes.length > 0 ? sizes.join(", ") : "mavjud emas",
    160,
  );
  const colorsText = truncateTelegramText(
  ...
  const fixedText = [
    ...
    `📏 Mavjud o‘lchamlar: ${sizesText}`,
    `🎨 Mavjud ranglar: ${colorsText}`,
    "",
